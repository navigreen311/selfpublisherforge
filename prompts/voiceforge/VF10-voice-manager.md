# VF10: Voice Manager Service

## Task
Create the Voice Manager service for voice catalog, custom cloning, and character voice assignment.

## Files to Create

### `backend/app/services/voiceforge/voice_manager.py`
...
SYSTEM_VOICES = [
    ...
    # Add ~13 more voices across genders, accents, age ranges, providers
...
class VoiceManager:
    ...
    async def get_system_voices(self, db: AsyncSession) -> list[AudiobookVoice]:
        ...
        result = await db.execute(
            select(AudiobookVoice).where(AudiobookVoice.is_system_voice == True)
        )
    ...
    async def get_org_voices(self, db: AsyncSession, org_id: uuid.UUID) -> list[AudiobookVoice]:
        ...
        result = await db.execute(
            select(AudiobookVoice).where(
                (AudiobookVoice.org_id == org_id) | (AudiobookVoice.is_system_voice == True)
            ).where(AudiobookVoice.active == True)
        )
        return list(result.scalars().all())

    async def create_clone(self, db: AsyncSession, org_id: uuid.UUID, name: str, audio_samples: list[Path], provider: str = "coqui_xtts") -> AudiobookVoice:
        ...
        if total_seconds < self.settings.VOICE_CLONE_MIN_AUDIO_SECONDS:
            raise ValueError(f"Need at least {self.settings.VOICE_CLONE_MIN_AUDIO_SECONDS}s of audio, got {total_seconds:.0f}s")
    ...
    async def preview_voice(self, db: AsyncSession, voice_id: uuid.UUID, sample_text: str) -> dict:
        ...
        tts = TTSEngine()
        audio = await tts.preview_voice(sample_text[:200], voice.provider_voice_id or str(voice.id), voice.provider)
        return {"audio_url": str(audio.audio_path), "duration_seconds": audio.duration_seconds}
    ...
    async def detect_characters(self, chapter_text: str) -> list[dict]:
        ...
        # Use Claude to detect characters and their dialogue
        ...
        return [{"name": name, "dialogue_count": count} for name, count in sorted(characters.items(), key=lambda x: -x[1])]
...
## Conventions
- Use SQLAlchemy async patterns from existing services
- Import models from `app.models.audiobook`
- Lazy-import TTSEngine to avoid circular imports
