# FIX15: Wire Audiobook Studio List Page

## Task
Replace the placeholder `/audiobook-studio` page with a real implementation that shows the audiobook project list.

## File to Modify: `frontend/src/app/(dashboard)/audiobook-studio/page.tsx`

### Implementation
Read the existing file first, then replace with a full implementation:
...
import { useAudiobookProjects, useCreateAudiobookProject } from "@/modules/audiobook/hooks";
...
### Metadata (keep in a separate layout.tsx or use generateMetadata)
```typescript
...
Note: If the page uses "use client", metadata must be in a parent layout or a separate `metadata` export won't work. Either remove "use client" and use a server component wrapper, or put metadata in the layout.

## Conventions
- Use existing shadcn/ui components
- Follow the project's page patterns
