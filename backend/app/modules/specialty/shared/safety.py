...
_SENSITIVITY_PATTERNS: dict[str, list[tuple[re.Pattern[str], str, str]]] = {
    "violence": [
        (
            ...
            "medium",
            "Replace with non-weapon alternatives",
        ),
        (
            re.compile(r"\b(sword|dagger|knife|blade|axe)\b", re.I),
            "low",
            "Consider age-appropriateness of weapon references",
        ),
        (
            re.compile(
                r"\b(kill|killing|murder|stab|shoot|shooting|attack|fight|fighting|punch|kick|blood|bleeding|wound|wounded)\b",
                re.I,
            ),
            "medium",
            "Avoid violent language; use conflict resolution themes",
        ),
        (
            re.compile(r"\b(bomb|explosion|explode|grenade|missile|warfare)\b", re.I),
            "high",
            "Remove military/explosive references",
        ),
    ],
    "weapons": [
        (
            ...
            "high",
            "Remove all weapon references for children's content",
        ),
        (
            re.compile(r"\b(crossbow|bow and arrow|spear|lance)\b", re.I),
            "low",
            "Consider context; may be acceptable in fantasy settings",
        ),
    ],
    "fear_horror": [
        (
            re.compile(
                r"\b(monster|demon|devil|evil|ghost|haunted|zombie|vampire|skeleton|skull|death|dead|die|dying|cemetery|graveyard)\b",
                re.I,
            ),
            "medium",
            "Evaluate fear intensity for target age range",
        ),
        (
            re.compile(r"\b(nightmare|terror|terrify|terrifying|horror|scream|screaming|blood|gore|gory)\b", re.I),
            "high",
            "Too intense for young children; tone down or remove",
        ),
        (
            re.compile(r"\b(dark|darkness|shadow|scary|creepy|spooky)\b", re.I),
            "low",
            "May be acceptable depending on age range and context",
        ),
    ],
    "stereotypes": [
        (
            ...
            "medium",
            "Use culturally respectful alternatives",
        ),
        (
            re.compile(r"\b(chief|medicine man|spirit animal|pow\s*wow)\b", re.I),
            "medium",
            "Avoid cultural appropriation; use respectful language",
        ),
        (
            re.compile(r"\b(crazy|insane|psycho|lunatic|schizo)\b", re.I),
            "low",
            "Avoid mental health stigma; use precise language",
        ),
    ],
    "mature_themes": [
        (
            ...
            "medium",
            "Not appropriate for children's content",
        ),
        (re.compile(r"\b(naked|nude|nudity|undress|strip|topless)\b", re.I), "high", "Remove nudity references"),
        (
            re.compile(r"\b(pregnant|pregnancy|birth|giving birth|labor|childbirth)\b", re.I),
            "low",
            "May require sensitive handling for young audiences",
        ),
    ],
    "substance_references": [
        (
            ...
            "medium",
            "Remove alcohol references for children's content",
        ),
        (
            re.compile(r"\b(smoking|cigarette|cigar|tobacco|vape|vaping)\b", re.I),
            "high",
            "Remove all smoking/tobacco references",
        ),
        (
            re.compile(r"\b(drug|drugs|pill|pills|needle|inject|syringe)\b", re.I),
            "high",
            "Remove drug references (unless medical context)",
        ),
    ],
}
