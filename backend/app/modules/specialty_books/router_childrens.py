...
@router.post("/", response_model=ChildrensBookResponse, status_code=status.HTTP_201_CREATED, summary="Create children's book")
async def create_book(
    body: ChildrensBookCreate, current_user: dict = Depends(get_current_user), db: AsyncSession = Depends(get_db),
):
    try:
...
async def update_book(
    book_id: UUID, body: ChildrensBookUpdate,
    current_user: dict = Depends(get_current_user), db: AsyncSession = Depends(get_db),
):
...
async def delete_book(book_id: UUID, current_user: dict = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
@router.post("/{book_id}/pages", response_model=PageResponse, status_code=status.HTTP_201_CREATED, summary="Create page")
async def create_page(
    book_id: UUID, body: PageCreate,
    current_user: dict = Depends(get_current_user), db: AsyncSession = Depends(get_db),
):
...
async def update_page(
    book_id: UUID, page_id: UUID, body: PageUpdate,
    current_user: dict = Depends(get_current_user), db: AsyncSession = Depends(get_db),
):
...
async def reorder_pages(
    book_id: UUID, body: PageReorderRequest,
    current_user: dict = Depends(get_current_user), db: AsyncSession = Depends(get_db),
):
...
async def upload_page_image(
    book_id: UUID, page_id: UUID, file: UploadFile = File(...),
    current_user: dict = Depends(get_current_user), db: AsyncSession = Depends(get_db),
):
...
async def list_characters(book_id: UUID, current_user: dict = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
@router.post("/{book_id}/characters", response_model=CharacterResponse, status_code=status.HTTP_201_CREATED, summary="Create character")
async def create_character(
    book_id: UUID, body: CharacterCreate,
    current_user: dict = Depends(get_current_user), db: AsyncSession = Depends(get_db),
):
...
async def update_character(
    book_id: UUID, char_id: UUID, body: CharacterUpdate,
    current_user: dict = Depends(get_current_user), db: AsyncSession = Depends(get_db),
):
    result = await svc.update_character(db, book_id, char_id, current_user["org_id"], body.model_dump(exclude_unset=True))
    if not result:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Character not found.")
    return result
