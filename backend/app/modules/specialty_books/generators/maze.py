    ...
    for r in range(size):
        for c in range(size):
            if ((r - cy) / ry) ** 2 + ((c - cx) / rx) ** 2 <= 1.0:
                mask[r][c] = True
    sh = max(1, size // 10)
    st = max(0, int(cy - ry) - max(1, size // 8))
    sb = max(0, int(cy - ry))
    for r in range(st, sb + 1):
        for c in range(size):
            if abs(c - cx) <= sh and 0 <= r < size:
                mask[r][c] = True
    return mask
...
def _make_mask_heart(size):
    ...
    for r in range(size):
        for c in range(size):
            x = (c - cx) / scale
            y = -(r - cy) / scale
            if (x * x + y * y - 1) ** 3 - x * x * y * y * y <= 0.05:
                mask[r][c] = True
    return mask


_MASK_BUILDERS: dict[str, Any] = {
    "rectangle": _make_mask_rectangle,
    "circle": _make_mask_circle,
    "star": _make_mask_star,
    "christmas_tree": _make_mask_christmas_tree,
    "pumpkin": _make_mask_pumpkin,
    "heart": _make_mask_heart,
}
...
def _count_dead_ends(grid, size):
    ...
    for r in range(size):
        for c in range(size):
            if grid[r][c]["masked"]:
                continue
            if sum(1 for w in ("top", "right", "bottom", "left") if not grid[r][c][w]) == 1:
                count += 1
    return count
...
def _difficulty_score(pl, de, bf, size):
    tc = size * size
    pr = pl / tc if tc else 0
    dr = de / tc if tc else 0
...
def _content_hash(grid, size, shape):
    ...
    for r in range(size):
        ...
        for c in range(size):
            cell = grid[r][c]
            rd.append((cell["top"], cell["right"], cell["bottom"], cell["left"], cell["masked"]))
        wd.append(rd)
...
def generate_maze(grid_size=20, difficulty="medium", shape="rectangle", seed=None):
    ...
    if grid_size == 20:
        grid_size = GRID_SIZE_PRESETS.get(difficulty, 20)
    mask = _MASK_BUILDERS[shape](grid_size)
    grid = _generate_maze_grid(grid_size, mask, rng)
    start, end = _find_start_end(grid_size, mask)
    sp = _solve_bfs(grid, grid_size, start, end)
    de = _count_dead_ends(grid, grid_size)
    bf = _branch_factor(grid, grid_size)
    score = _difficulty_score(len(sp), de, bf, grid_size)
    ch = _content_hash(grid, grid_size, shape)
    return {
        "grid": [[grid[r][c] for c in range(grid_size)] for r in range(grid_size)],
        "size": grid_size,
        "start": list(start),
        "end": list(end),
        "solution_path": [list(p) for p in sp],
        "dead_ends": de,
