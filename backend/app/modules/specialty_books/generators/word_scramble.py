...
def _is_real_word(word):
    return word.upper() in _load_dictionary()
...
def _scramble_word(word, rng, max_attempts=100):
    ...
    for _ in range(max_attempts):
        ...
        rng.shuffle(s)
        r = "".join(s)
        if r != word.upper() and not _is_real_word(r):
            return r
    for _ in range(max_attempts):
        s = letters[:]
        rng.shuffle(s)
...
def generate_word_scramble(words, difficulty="medium", category=None, seed=None):
    ...
    if not scrambles:
        raise ValueError("Could not scramble any of the provided words.")
    ...
    ch = hashlib.sha256(
        json.dumps({"scrambles": [(s["original"], s["scrambled"]) for s in scrambles]}, sort_keys=True).encode()
    ).hexdigest()
