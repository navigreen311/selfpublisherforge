...
def _apply_cipher(phrase, cm):
    ...
    for ch in phrase:
        if ch.upper() in cm:
            e = cm[ch.upper()]; result.append(e if ch.isupper() else e.lower())
...
def generate_cryptogram(phrase, difficulty="medium", seed=None):
    ...
    rng = random.Random(seed); cm = _generate_cipher_map(rng)
    encoded = _apply_cipher(phrase, cm); inv = {v:k for k,v in cm.items()}
    used = set(ch.upper() for ch in phrase if ch.isalpha())
    eu = [cm[l] for l in used if l in cm]
    hints = []
    if difficulty == "easy":
        n = min(3, len(eu)); chosen = rng.sample(eu, n) if len(eu)>=n else eu
        hints = [{"encoded_letter":e,"decoded_letter":inv[e]} for e in chosen]
    elif difficulty == "medium" and eu:
        c = rng.choice(eu); hints = [{"encoded_letter":c,"decoded_letter":inv[c]}]
    ...
    ch_hash = hashlib.sha256(json.dumps({"original":phrase,"encoded":encoded,"cipher_map":cm},sort_keys=True).encode()).hexdigest()
    return {"original":phrase,"encoded":encoded,"cipher_map":cm,"hints":hints,
            "letter_frequency":freq,"difficulty_score":score,"content_hash":ch_hash}
