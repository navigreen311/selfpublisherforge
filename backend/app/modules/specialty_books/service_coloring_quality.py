...
async def _get_book(db: AsyncSession, book_id: uuid.UUID, org_id: uuid.UUID) -> ColoringBook:
    result = await db.execute(select(ColoringBook).where(
...
async def _get_page(db: AsyncSession, book_id: uuid.UUID, page_id: uuid.UUID, org_id: uuid.UUID) -> ColoringBookPage:
    result = await db.execute(select(ColoringBookPage).where(
        ColoringBookPage.id == page_id, ColoringBookPage.book_id == book_id,
        ColoringBookPage.org_id == org_id, ColoringBookPage.deleted_at.is_(None)))
...
async def _get_book_pages(db: AsyncSession, book_id: uuid.UUID, org_id: uuid.UUID) -> list[ColoringBookPage]:
    result = await db.execute(select(ColoringBookPage).where(
        ColoringBookPage.book_id == book_id, ColoringBookPage.org_id == org_id,
        ColoringBookPage.deleted_at.is_(None)).order_by(ColoringBookPage.page_number))
    return list(result.scalars().all())


async def generate_line_art(db: AsyncSession, book_id: uuid.UUID, page_id: uuid.UUID, org_id: uuid.UUID, prompt: str) -> GenerateLineArtResponse:
...
def _step_auto_clean(page_data: dict[str, Any]) -> PipelineStepResult:
    ...
    passed = gray_pct < 5.0
    return PipelineStepResult(step=PipelineStepName.AUTO_CLEAN, passed=passed, details=f"Gray artifacts: {gray_pct:.1f}% (threshold <5%)", before_value=gray_pct, after_value=0.0 if passed else gray_pct, duration_ms=45)
...
def _step_stroke_uniformity(page_data: dict[str, Any]) -> PipelineStepResult:
    ...
    passed = ratio <= 3.0
    return PipelineStepResult(step=PipelineStepName.STROKE_UNIFORMITY, passed=passed, details=f"Stroke ratio: {ratio:.1f}x (threshold <=3x)", before_value=ratio, after_value=min(ratio, 3.0), duration_ms=60)
...
def _step_closed_shapes(page_data: dict[str, Any]) -> PipelineStepResult:
    ...
    passed = open_count == 0
    return PipelineStepResult(step=PipelineStepName.CLOSED_SHAPES, passed=passed, details=f"Open shapes: {open_count}/{total_shapes}", before_value=open_count, after_value=0 if passed else open_count, duration_ms=80)
...
def _step_speck_removal(page_data: dict[str, Any]) -> PipelineStepResult:
    ...
    return PipelineStepResult(step=PipelineStepName.SPECK_REMOVAL, passed=passed, details=f"Specks removed: {speck_count} (area < {SPECK_MIN_AREA_PX}px)", before_value=speck_count, after_value=0, duration_ms=35)
...
def _step_background(page_data: dict[str, Any]) -> PipelineStepResult:
    ...
    return PipelineStepResult(step=PipelineStepName.BACKGROUND, passed=passed, details=f"Background: {bg_color}, uniformity: {bg_uni:.1f}%", before_value=bg_color, after_value="#FFFFFF", duration_ms=20)
...
async def run_quality_pipeline(db: AsyncSession, book_id: uuid.UUID, page_id: uuid.UUID, org_id: uuid.UUID) -> QualityPipelineResponse:
    ...
    gen = PipelineStepResult(step=PipelineStepName.GENERATE, passed=True, details="Illustration already generated", duration_ms=0)
    steps_2_6 = [_step_auto_clean(page_data), _step_stroke_uniformity(page_data), _step_closed_shapes(page_data), _step_speck_removal(page_data), _step_background(page_data)]
    qc = _step_quality_check(steps_2_6)
    all_steps = [gen] + steps_2_6 + [qc]
    overall_passed = qc.passed
    cleaned_url = f"https://cdn.selfpublisherforge.com/coloring/{book_id}/{page_id}/cleaned.png" if overall_passed else None
    page.qa_scores = {"pipeline_steps": {s.step.value: s.passed for s in all_steps}, "ink_coverage_pct": page_data.get("ink_coverage_pct", 0), "complexity_score": page_data.get("complexity_score", 0)}
...
def check_ink_density(page_data: dict[str, Any]) -> InkDensityResult:
    ...
    if ink_pct > INK_DENSITY_THRESHOLD_PCT:
        heavy.append({"region": "full_page", "coverage_pct": ink_pct, "recommendation": "Reduce fill areas or simplify line work"})
    return InkDensityResult(page_id=page_id, ink_coverage_pct=round(ink_pct, 2), passed=ink_pct <= INK_DENSITY_THRESHOLD_PCT, threshold_pct=INK_DENSITY_THRESHOLD_PCT, heavy_regions=heavy)
...
async def detect_duplicates(db: AsyncSession, book_id: uuid.UUID, org_id: uuid.UUID) -> DuplicateDetectionResponse:
    ...
    hashes = [(p.id, _compute_phash(p.illustration_prompt or "", p.illustration_seed)) for p in pages if p.illustration_url]
    pairs = []
    for i in range(len(hashes)):
        for j in range(i + 1, len(hashes)):
            sim = _structural_similarity(hashes[i][1], hashes[j][1])
            if sim >= DUPLICATE_SIMILARITY_THRESHOLD:
                pairs.append(DuplicatePair(page_a_id=hashes[i][0], page_b_id=hashes[j][0], similarity_score=round(sim, 2), method="phash+ssim"))
    return DuplicateDetectionResponse(book_id=book_id, total_pages=len(pages), duplicate_pairs=pairs, has_duplicates=len(pairs) > 0)
...
async def calculate_theme_cohesion(db: AsyncSession, book_id: uuid.UUID, org_id: uuid.UUID) -> ThemeCohesionResponse:
    ...
    pages = await _get_book_pages(db, book_id, org_id)
    if len(pages) < 2:
        return ThemeCohesionResponse(book_id=book_id, cohesion_score=100.0, analysis="Insufficient pages for cohesion analysis")
    ...
    return ThemeCohesionResponse(book_id=book_id, cohesion_score=round(cohesion, 1), analysis=f"Analyzed {len(pages)} pages, {len(outliers)} outliers detected", outlier_page_ids=outliers)


async def generate_coloring_simulation(db: AsyncSession, book_id: uuid.UUID, page_id: uuid.UUID, org_id: uuid.UUID, medium: ColoringMedium) -> ColoringSimulationResponse:
...
async def get_complexity_histogram(db: AsyncSession, book_id: uuid.UUID, org_id: uuid.UUID) -> ComplexityHistogramResponse:
    ...
    return ComplexityHistogramResponse(book_id=book_id, total_pages=len(pages), distribution=[ComplexityBucketCount(bucket=b, count=len(ids), page_ids=ids) for b, ids in buckets.items()])


async def batch_generate(db: AsyncSession, book_id: uuid.UUID, org_id: uuid.UUID, descriptions: list[dict[str, Any]], variation_mode: VariationMode = VariationMode.MILD, auto_qa: bool = True) -> BatchGenerateResponse:
    ...
    job = ColoringBatchJob(org_id=org_id, book_id=book_id, batch_config={"descriptions": descriptions, "variation_mode": variation_mode.value, "auto_qa": auto_qa}, budget_limit_cents=cost * 2, status="queued", pages_total=total, pages_completed=0)
    db.add(job)
    await db.flush()
    cnt_result = await db.execute(select(func.count(ColoringBookPage.id)).where(ColoringBookPage.book_id == book_id, ColoringBookPage.deleted_at.is_(None)))
...
async def get_batch_status(db: AsyncSession, job_id: uuid.UUID, org_id: uuid.UUID) -> BatchStatusResponse:
    ...
    result = await db.execute(select(ColoringBatchJob).where(ColoringBatchJob.id == job_id, ColoringBatchJob.org_id == org_id, ColoringBatchJob.deleted_at.is_(None)))
    ...
    if job.book_id:
        for p in await _get_book_pages(db, job.book_id, org_id):
            ...
            if s.get("batch_job_id") == str(job_id):
                ...
                if st == "failed":
                    pages_failed += 1
                page_statuses.append(BatchPageStatus(page_number=p.page_number, page_id=p.id, status=st, qa_passed=p.qa_passed, error=s.get("error")))
    total, completed = job.pages_total or 0, job.pages_completed or 0
    return BatchStatusResponse(job_id=job_id, status=job.status if isinstance(job.status, str) else str(job.status), progress_pct=round((completed / total * 100) if total > 0 else 0.0, 1), total_pages=total, pages_completed=completed, pages_failed=pages_failed, page_statuses=page_statuses, estimated_cost_cents=total * COST_PER_PAGE_CENTS, spent_cents=job.spent_cents or 0)
...
async def book_quality_dashboard(db: AsyncSession, book_id: uuid.UUID, org_id: uuid.UUID) -> QualityDashboardResponse:
    ...
    spc: dict[str, int] = {"auto_clean": 0, "stroke_uniformity": 0, "closed_shapes": 0, "speck_removal": 0, "background": 0}
    inks: list[float] = []
    for p in pages:
        qa = p.qa_scores or {}
    ...
    pq = PrintQualitySummary(line_quality_score=round(lq, 1), closed_shapes_score=round(cs, 1), stroke_uniformity_score=round(su, 1), ink_density_score=round(ids, 1), small_areas_score=round(sa, 1))
    buckets: dict[ComplexityBucket, list[uuid.UUID]] = {b: [] for b in ComplexityBucket}
    for p in pages:
        buckets[_score_to_bucket(_analyze_page_data(p).get("complexity_score", 50))].append(p.id)
    ...
    overall = lq * 0.2 + cs * 0.2 + su * 0.15 + ids * 0.15 + sa * 0.1 + coh.cohesion_score * 0.1 + (100 if not dup.has_duplicates else 50) * 0.1
    ...
    return QualityDashboardResponse(book_id=book_id, overall_score=round(overall, 1), total_pages=total, pages_passed=passed_count, pages_failed=failed_count, complexity_distribution=cd, theme_cohesion=coh.cohesion_score, print_quality=pq, duplicate_pairs_count=len(dup.duplicate_pairs), recommendations=recs)
