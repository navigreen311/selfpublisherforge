...
def _log_startup_config_warnings() -> None:
    ...
    if not sendgrid_configured and not smtp_configured:
        logger.warning(
            "No email provider configured (neither SendGrid nor SMTP) — " "transactional emails will not be sent"
        )
    ...
    integrations = {
        ...
        "Stripe billing tiers": all(
            [
                s.STRIPE_PRICE_STARTER,
                s.STRIPE_PRICE_PRO,
                s.STRIPE_PRICE_BUSINESS,
                s.STRIPE_PRICE_ENTERPRISE,
            ]
        ),
