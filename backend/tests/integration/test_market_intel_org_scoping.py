...
class TestUnauthenticatedRejection:
    ...
    async def test_keyword_research_requires_auth(self, unauthenticated_client: AsyncClient):
        resp = await unauthenticated_client.post(
            f"{BASE}/keywords/research", json={"keywords": ["test"]}
        )
        assert resp.status_code in (401, 403)
    ...
    async def test_keyword_suggestions_requires_auth(self, unauthenticated_client: AsyncClient):
        resp = await unauthenticated_client.get(
            f"{BASE}/keywords/suggestions", params={"genre": "romance"}
        )
        assert resp.status_code in (401, 403)
    ...
    async def test_niche_analysis_requires_auth(self, unauthenticated_client: AsyncClient):
        resp = await unauthenticated_client.post(
            f"{BASE}/analyze-niche", json={"niche": "self-help tips"}
        )
        assert resp.status_code in (401, 403)
    ...
    async def test_competitor_track_requires_auth(self, unauthenticated_client: AsyncClient):
        resp = await unauthenticated_client.post(
            f"{BASE}/competitors/track", json={"asin": "B000000001"}
        )
        assert resp.status_code in (401, 403)
...
class TestCompetitorListOrgScoping:
    ...
    @pytest.mark.asyncio
    async def test_list_competitors_scoped_to_org_a(
    ...
    @pytest.mark.asyncio
    async def test_list_competitors_scoped_to_org_b(
        self, db_session: AsyncSession, client_org_b: AsyncClient
    ):
    ...
    @pytest.mark.asyncio
    async def test_list_competitors_empty_for_new_org(
        self, db_session: AsyncSession, client_org_b: AsyncClient
    ):
...
class TestCompetitorTrackOrgScoping:
    ...
    @pytest.mark.asyncio
    async def test_track_competitor_assigns_org_id(
        self, db_session: AsyncSession, client_org_a: AsyncClient
    ):
        """Tracking a competitor stamps it with the caller's org_id."""
        resp = await client_org_a.post(
            f"{BASE}/competitors/track", json={"asin": "B000000005"}
        )
    ...
    async def test_track_competitor_not_visible_to_other_org(
        ...
        track_resp = await client_org_a.post(
            f"{BASE}/competitors/track", json={"asin": "B000000006"}
        )
    ...
    async def test_track_duplicate_asin_same_org_returns_existing(
        ...
        resp1 = await client_org_a.post(
            f"{BASE}/competitors/track", json={"asin": "B000000009"}
        )
        assert resp1.status_code == 201
        id1 = resp1.json()["id"]

        resp2 = await client_org_a.post(
            f"{BASE}/competitors/track", json={"asin": "B000000009"}
        )
...
class TestCompetitorGetByIdScoping:
    ...
    @pytest.mark.asyncio
    async def test_owner_can_get_own_competitor_by_id(
        self, db_session: AsyncSession, client_org_a: AsyncClient
    ):
    ...
    @pytest.mark.asyncio
    async def test_nonexistent_competitor_returns_404(
...
class TestSnapshotOrgScoping:
    ...
    @pytest.mark.asyncio
    async def test_snapshots_currently_unscoped(
        self, db_session: AsyncSession, client_org_a: AsyncClient
    ):
    ...
    @pytest.mark.asyncio
    async def test_snapshots_limit_parameter(
        self, db_session: AsyncSession, client_org_a: AsyncClient
    ):
...
class TestKeywordResearchOrgScope:
    ...
    async def test_keyword_suggestions_with_org_b(self, client_org_b: AsyncClient):
        ...
        resp = await client_org_b.get(
            f"{BASE}/keywords/suggestions", params={"genre": "romance", "limit": 3}
        )
        assert resp.status_code == 200
        data = resp.json()
        assert isinstance(data, list)
