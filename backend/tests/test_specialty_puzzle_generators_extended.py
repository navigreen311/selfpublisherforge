...
class TestMazeGenerator:
    ...
    def test_maze_rectangle_shape(self):
        for row in generate_maze(grid_size=10, shape="rectangle", seed=1)["grid"]:
            for cell in row: assert cell["masked"] is False
    ...
    def test_maze_star_shape(self):
        assert sum(1 for row in generate_maze(grid_size=20, shape="star", seed=5)["grid"] for c in row if c["masked"]) > 0
    ...
    def test_maze_content_hash_deterministic(self):
        assert generate_maze(grid_size=10, seed=99)["content_hash"] == generate_maze(grid_size=10, seed=99)["content_hash"]
    def test_maze_invalid_shape_raises(self):
        with pytest.raises(ValueError, match="Invalid shape"): generate_maze(shape="hexagon")

class TestSudokuGenerator:
    def test_sudoku_solution_valid_rows(self):
        for row in generate_sudoku(grid_size=9, seed=42)["solution"]: assert sorted(row) == list(range(1,10))
    def test_sudoku_solution_valid_cols(self):
        r = generate_sudoku(grid_size=9, seed=42)
        for c in range(9): assert sorted(r["solution"][row][c] for row in range(9)) == list(range(1,10))
    def test_sudoku_solution_valid_boxes(self):
        r = generate_sudoku(grid_size=9, seed=42)
        for br in range(0,9,3):
            for bc in range(0,9,3):
                assert sorted(r["solution"][row][c] for row in range(br,br+3) for c in range(bc,bc+3)) == list(range(1,10))
    ...
    def test_sudoku_4x4_variant(self):
        ...
        assert r["size"] == 4
        for row in r["solution"]: assert sorted(row) == [1,2,3,4]
    def test_sudoku_6x6_variant(self):
        r = generate_sudoku(grid_size=6, difficulty="medium", seed=5)
        assert r["size"] == 6
        for row in r["solution"]: assert sorted(row) == [1,2,3,4,5,6]
    def test_sudoku_content_hash_deterministic(self):
        assert generate_sudoku(grid_size=4, seed=77)["content_hash"] == generate_sudoku(grid_size=4, seed=77)["content_hash"]
...
class TestWordScrambleGenerator:
    ...
    def test_scramble_content_hash_deterministic(self):
        assert generate_word_scramble(words=["ABC","DEF"], seed=10)["content_hash"] == generate_word_scramble(words=["ABC","DEF"], seed=10)["content_hash"]
    def test_scramble_invalid_words_filtered(self):
        o = [s["original"] for s in generate_word_scramble(words=["GOOD","","123","OK!!!","FINE"], seed=1)["scrambles"]]
...
class TestCryptogramGenerator:
    def test_no_letter_maps_to_itself(self):
        for k,v in generate_cryptogram("The quick brown fox", seed=42)["cipher_map"].items(): assert k != v
    def test_can_decode_back_to_original(self):
        phrase = "HELLO WORLD"; r = generate_cryptogram(phrase, seed=42)
    ...
    def test_cryptogram_hints_easy(self):
        assert 2 <= len(generate_cryptogram("The quick brown fox jumps over the lazy dog", difficulty="easy", seed=42)["hints"]) <= 3
    def test_cryptogram_hints_hard(self):
        assert len(generate_cryptogram("The quick brown fox jumps over the lazy dog", difficulty="hard", seed=42)["hints"]) == 0
    def test_cryptogram_content_hash_deterministic(self):
        assert generate_cryptogram("Test phrase", seed=55)["content_hash"] == generate_cryptogram("Test phrase", seed=55)["content_hash"]
...
class TestSVGRenderers:
    def _v(self, s): ET.fromstring(s)
    def test_maze_svg_valid_xml(self): self._v(render_maze_svg(generate_maze(grid_size=10, seed=1)))
    def test_maze_svg_with_solution(self):
        s = render_maze_svg(generate_maze(grid_size=10, seed=1), show_solution=True); self._v(s); assert "polyline" in s
    def test_sudoku_svg_valid_xml(self): self._v(render_sudoku_svg(generate_sudoku(grid_size=9, seed=1)))
    def test_sudoku_svg_with_solution(self): self._v(render_sudoku_svg(generate_sudoku(grid_size=9, seed=1), show_solution=True))
    def test_word_scramble_svg_valid_xml(self): self._v(render_word_scramble_svg(generate_word_scramble(words=["HELLO","WORLD"], seed=1)))
    def test_cryptogram_svg_valid_xml(self): self._v(render_cryptogram_svg(generate_cryptogram("Test phrase here", seed=1)))
