...
These tests exercise connection tracking, broadcasting, personal messaging,
and Redis pub/sub integration using mocks — no live Redis required.
"""
...
class FakeWebSocket:
    ...
    async def receive_json(self) -> dict[str, Any]:
        # Block forever in tests — the handler loop is not exercised here.
...
@pytest.mark.asyncio
async def test_send_personal_to_closed_ws() -> None:
    mgr = ConnectionManager()
...
async def test_broadcast_publishes_to_redis() -> None:
    """When a Redis URL is configured, broadcast() should publish the message
    to the appropriate Redis pub/sub channel."""
...
async def test_broadcast_falls_back_to_local_without_redis() -> None:
    """Without Redis, broadcast() should send directly to local connections."""
...
async def test_shutdown_cleans_up() -> None:
    ...
    await mgr.shutdown()
    # Should not raise and heartbeat task should be done.
