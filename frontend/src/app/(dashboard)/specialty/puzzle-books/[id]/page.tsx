...
export default function PuzzleBookEditorPage() {
  ...
  const buildMutationInput = useCallback(() => {
    const wl = editWordList.split("\n").map((w) => w.trim()).filter(Boolean);
    return {
      puzzle_type: editPuzzleType,
      difficulty: editDifficulty,
      grid_size: editGridSize || undefined,
      theme: editTheme || undefined,
      word_list: wl.length > 0 ? wl : undefined,
    };
  }, [editPuzzleType, editDifficulty, editGridSize, editTheme, editWordList]);

  const handleGenerate = useCallback(() => generatePuzzle.mutate(buildMutationInput()), [generatePuzzle, buildMutationInput]);
  const handleRegenerate = useCallback(() => { if (selectedPuzzle) generatePuzzle.mutate(buildMutationInput()); }, [selectedPuzzle, generatePuzzle, buildMutationInput]);
  ...
  if (showReviews) {
    return (
      <div className="container mx-auto py-6 space-y-4">
        <Button variant="ghost" size="sm" className="gap-1.5" onClick={() => setShowReviews(false)}>
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)]">
      ...
      <div className="border-b px-4 py-3 flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-2">
          <Button variant="ghost" size="sm" onClick={() => setShowReviews(true)} className="gap-1.5">
            <MessageSquareWarning className="h-4 w-4" />
            Reviews
          </Button>
          <Button variant="outline" size="sm" onClick={() => exportBook.mutate({ format: "pdf", include_answers: true })} disabled={exportBook.isPending}>
      ...
      <div className="flex flex-1 overflow-hidden">
          ...
          <div className="p-3 border-b flex items-center justify-between">
            ...
            <Button
              ...
              onClick={() => {
                ...
                setEditPuzzleType((book.puzzle_config[0]?.type as PuzzleType) ?? "word_search");
                setEditWordList("");
              }}
              title="New puzzle"
            >
                  ...
                  >
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-xs font-medium truncate">
                        #{puzzle.puzzle_number} {PUZZLE_TYPE_LABELS[puzzle.puzzle_type] ?? puzzle.puzzle_type}
                      </span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <Badge variant="outline" className={cn("text-[9px] capitalize px-1 py-0", DIFFICULTY_COLORS[puzzle.difficulty] ?? "")}>
                        {puzzle.difficulty}
                      </Badge>
                      <Badge variant="secondary" className={cn("text-[9px] capitalize px-1 py-0", STATUS_COLORS[puzzle.status] ?? "")}>
                        {puzzle.status}
                      </Badge>
                    </div>
                    {puzzle.theme && (
                      <p className="text-[10px] text-muted-foreground mt-1 truncate">{puzzle.theme}</p>
                    )}
                  </button>
                );
              })
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          <div className="p-3 border-b flex items-center justify-between">
            <span className="text-sm font-medium">
              {selectedPuzzle
                ? `Preview \u2014 #${selectedPuzzle.puzzle_number} ${PUZZLE_TYPE_LABELS[selectedPuzzle.puzzle_type]}`
                : "Puzzle Preview"}
            </span>
            {selectedPuzzle && (
              <div className="flex items-center gap-1.5">
                <Label htmlFor="answer-key-center" className="text-xs">Answer Key</Label>
                <Switch id="answer-key-center" checked={showAnswerKey} onCheckedChange={setShowAnswerKey} />
              </div>
            )}
          </div>
            ...
            {selectedPuzzle ? (
              <div className="max-w-2xl mx-auto space-y-4">
                    ...
                    {showAnswerKey && selectedPuzzle.solution_data ? (
                      <div>
                        <div className="flex items-center gap-2 mb-3">
                          ...
                          <span className="text-sm font-medium text-green-600">Answer Key</span>
                        </div>
                      ...
                      <div>
                        <div className="flex items-center gap-2 mb-3">
                          ...
                          {selectedPuzzle.grid_size && (
                            <Badge variant="outline" className="text-[10px]">{selectedPuzzle.grid_size}</Badge>
                          )}
                        </div>
                ...
                {selectedPuzzle.clues && selectedPuzzle.clues.length > 0 && (
                  <Card>
                    <CardContent className="p-4">
                      <span className="text-sm font-medium mb-2 block">Clues ({selectedPuzzle.clues.length})</span>
                      <div className="space-y-1">
                        {selectedPuzzle.clues.map((clue, idx) => (
                          <div key={idx} className="flex items-baseline gap-2 text-xs">
                            ...
                            <span>{clue.clue}</span>
                            {showAnswerKey && (
                              <span className="text-muted-foreground ml-auto font-mono">({clue.answer})</span>
                            )}
                          </div>
                        ))}
                      </div>
                ...
                {selectedPuzzle.quality_issues && selectedPuzzle.quality_issues.length > 0 && (
              ...
              <div className="flex flex-col items-center justify-center h-full text-center">
                ...
                <p className="text-muted-foreground text-sm mb-1">
                  {allPuzzles.length === 0 ? "No puzzles generated yet" : "Select a puzzle from the left panel"}
                </p>
        ...
        <div className="w-72 border-l flex flex-col shrink-0 overflow-y-auto">
          <div className="p-3 border-b">
            <span className="text-sm font-medium">{selectedPuzzle ? "Puzzle Settings" : "New Puzzle"}</span>
          </div>
          <div className="p-3 space-y-4">
            <div className="space-y-1.5">
              <Label className="text-xs">Puzzle Type</Label>
              <Select value={editPuzzleType} onValueChange={(v) => setEditPuzzleType(v as PuzzleType)}>
            ...
            <div className="space-y-1.5">
              ...
              <Input value={editTheme} onChange={(e) => setEditTheme(e.target.value)} placeholder="e.g., Animals, Space..." className="h-8 text-xs" />
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                <Label htmlFor="diagonal-toggle" className="text-xs text-muted-foreground">Allow diagonal</Label>
                <Switch id="diagonal-toggle" checked={allowDiagonal} onCheckedChange={setAllowDiagonal} />
              </div>
              <div className="flex items-center justify-between">
                <Label htmlFor="backwards-toggle" className="text-xs text-muted-foreground">Allow backwards</Label>
                <Switch id="backwards-toggle" checked={allowBackwards} onCheckedChange={setAllowBackwards} />
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              />
              <p className="text-[10px] text-muted-foreground">{wordCount} words</p>
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <Switch checked={showAnswerKey} onCheckedChange={setShowAnswerKey} disabled={!selectedPuzzle} />
            </div>
            ...
            <div className="space-y-2">
              {selectedPuzzle ? (
                <Button size="sm" className="w-full" onClick={handleRegenerate} disabled={generatePuzzle.isPending}>
                  <RefreshCw className={cn("h-4 w-4 mr-2", generatePuzzle.isPending && "animate-spin")} />
                  {generatePuzzle.isPending ? "Regenerating..." : "Regenerate"}
                </Button>
              ) : (
                <Button size="sm" className="w-full" onClick={handleGenerate} disabled={generatePuzzle.isPending}>
                  <Plus className={cn("h-4 w-4 mr-2", generatePuzzle.isPending && "animate-spin")} />
