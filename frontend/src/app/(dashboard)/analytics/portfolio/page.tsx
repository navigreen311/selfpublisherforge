    ...
    return (
      <div className="space-y-6">
        ...
        <div className="bg-red-50 border border-red-200 rounded-md p-4">
          <p className="text-red-800">{t("portfolio.error")}</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="bg-gray-50 border border-gray-200 rounded-md p-4">
          <p className="text-gray-600">{t("portfolio.noData")}</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      {allBooks.length === 0 && (
        <div className="bg-white rounded-lg border border-gray-200 p-12 text-center">
          <h3 className="text-lg font-semibold text-gray-900 mb-2">{t("portfolio.noBooksTitle")}</h3>
          <p className="text-gray-600 mb-6">
            {t("portfolio.noBooksDescription")}
          </p>
          <Link
            href="/projects"
            className="inline-flex px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 font-medium"
          >
            {t("portfolio.goToProjects")}
          </Link>
        </div>
      )}
    </div>
